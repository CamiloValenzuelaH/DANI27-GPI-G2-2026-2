...
def _store_delivery_code(user_id: str, delivery_method: Literal["sms", "email"], code: str) -> None:
    key = _delivery_key(user_id, delivery_method)
    ttl = _delivery_ttl_seconds(delivery_method)
    hashed_value = hashlib.sha256(code.encode("utf-8")).hexdigest()

    print(f"\n💾 [REDIS GUARDANDO] Llave construida: '{key}' | TTL: {ttl}s")

    # Usamos .set con ex= explícito para que no falle según la versión de la librería redis
    _rate_limiter.set(key, hashed_value, ex=ttl)
    _rate_limiter.delete(_delivery_attempts_key(user_id, delivery_method))
    
    # Confirmación instantánea en la consola
    check = _rate_limiter.get(key)
    print(f"💾 [REDIS CONFIRMACIÓN] ¿Persistió el dato?: {check is not None} | Hash: {check}\n")
...
def _verify_delivery_code(user_id: str, delivery_method: Literal["sms", "email"], code: str) -> bool:
    ...
    # Al tener decode_responses=True al inicio del archivo, esto ya baja como string
    stored_hash = _rate_limiter.get(redis_key)

    print("\n--- 🔐 [AUDITORÍA VERIFICACIÓN 2FA] ---")
    print(f"🔑 Llave buscada: '{redis_key}'")
    print(f"📥 Hash en Redis: '{stored_hash}'")
    print(f"🤖 Código Front: '{code}'")

    if not stored_hash:
        print("❌ ERROR: La llave no existe en Redis o devolvió None.")
    ...
    if attempt_count >= _MAX_OTP_ATTEMPTS:
        print("❌ ERROR: Máximo de intentos alcanzado.")
        _clear_delivery_code(user_id, delivery_method)
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid or expired 2FA code")

    # Calculamos el hash del código que metió el usuario en la web
    candidate_hash = hashlib.sha256(code.encode("utf-8")).hexdigest()
    print(f"🧮 Hash candidato: '{candidate_hash}'")

    # Forzamos la comparación limpia en formato string
    if hmac.compare_digest(str(candidate_hash), str(stored_hash)):
        print("✅ ÉXITO: Los hashes coinciden plenamente.")
        _clear_delivery_code(user_id, delivery_method)
        print("----------------------------------------\n")
        return True

    print("❌ ERROR: El hash no coincide.")
    _rate_limiter.incr(attempts_key)
    raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid or expired 2FA code")
...
def verify_two_factor(data: TwoFactorVerifyRequest, db: Session, request: Request) -> TwoFactorVerifyResponse:
    ...
    if data.backup_code:
        ...
        try:
            print(f"📌 [DEBUG VERIFICAR] El user_id que se usa para BUSCAR es: '{user.id}' (Tipo: {type(user.id)})")
...
def send_email_code(data: TwoFactorDeliveryRequest, db: Session, request: Request) -> dict:
    ...
    code = f"{secrets.randbelow(1_000_000):06d}"
    print(f"📌 [DEBUG GENERAR] El user_id que se usará para GUARDAR es: '{user.id}' (Tipo: {type(user.id)})")
    ...
    try:
        ...
        print(f"❌ ERROR ENVIANDO EMAIL: {str(e)}") # 2. Esto saldrá en tu consola de Docker
        traceback.print_exc()
