...
def _render_report_text(
    ...
    summary_text = rendered.get("summary", "")
    ...
    if template == "soa":
        ...
        for control in rendered.get("controls", []):
            lines.append(f"- **{control['control']}**<br>Estado: *{control['status']}*<br>Justificación: {control['justification']}")
...
def generate_report(
    ...
    async def _generate_job() -> dict[str, Any]:
        ...
        # --- PLAN B: Extraemos el texto directamente de lo que mandó React ---
        custom_content = request_data.get("document_text")
        if custom_content:
            rendered["summary"] = custom_content 
        # ---------------------------------------------------------------------
        ...
        # Si React mandó texto, forzamos que use ese texto para el PDF
        if custom_content:
             text_content = _render_report_text(
                request_data.get("template"),
                {"summary": custom_content}, 
                request_data.get("title"),
                request_data.get("description"),
            )
        else:
