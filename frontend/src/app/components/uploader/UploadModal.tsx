...
const API_ROOT = (import.meta.env.VITE_API_URL ?? "http://localhost:8001/api/v1").replace(/\/api\/v1\/?$/, "") || "http://localhost:8001";
const EVIDENCE_UPLOAD_URL = `${API_ROOT}/api/v1/evidences`;
const EVIDENCE_CLASSIFY_URL = `${API_ROOT}/api/v1/evidences/classify`;
...
const buildClassificationPayload = (item: UploadItem) => ({
  evidence_id: item.uploadReference,
  file_name: item.file.name,
  file_path: item.uploadReference,
...
const classifyEvidence = async (item: UploadItem): Promise<unknown> => {
  const response = await requestWithAuth(EVIDENCE_CLASSIFY_URL, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify(buildClassificationPayload(item)),
  });
      ...
      <div className="grid grid-cols-1 gap-3 2xl:grid-cols-2">
        ...
        <label className="space-y-1">
          ...
          <Input
            ...
            placeholder="Documento, imagen, informe"
            className="border-white/10 bg-white/5 text-slate-100 placeholder:text-slate-500"
          />
        </label>
        <label className="space-y-1">
...
export function UploadModal({ open, onOpenChange, onComplete, questionId }: UploadModalProps) {
  ...
  const processItem = React.useCallback(
    async (item: UploadItem): Promise<UploadItem> => {
      ...
      const classificationResult = await retryAsync(async () => {
        const response = await classifyEvidence({
          ...item,
          uploadReference: readEvidenceReference(uploadResult) ?? item.file.name,
        });

        return readClassificationPreview(response);
      });
      ...
      <DialogContent className="h-[92vh] w-[96vw] max-w-[1400px] overflow-auto border-white/10 bg-slate-950 p-0 text-slate-50 shadow-[0_40px_120px_rgba(0,0,0,0.65)]">
        <div className="border-b border-white/10 bg-[linear-gradient(135deg,rgba(14,165,233,0.12),rgba(15,23,42,0.4))] px-6 py-5">
          <DialogHeader className="text-left">
            ...
            <DialogDescription className="max-w-3xl text-sm text-slate-300">
              Flujo DANI-FE-022: carga por archivo, clasificación ISO por backend y edición inline de metadata.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="mt-4 flex flex-wrap gap-2 text-xs text-slate-300">
            ...
            <Badge variant="outline" className="border-white/10 text-slate-200">POST /api/evidences/classify</Badge>
          </div>
        </div>
        ...
        <DialogFooter className="border-t border-white/10 bg-slate-950 px-6 py-4 sm:justify-between">
          <div className="flex items-center gap-2 text-xs text-slate-400">
            ...
            <span>La subida se hace por archivo y la clasificación se consulta inmediatamente después.</span>
          </div>
          <div className="flex items-center gap-2">
