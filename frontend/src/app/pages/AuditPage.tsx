  ...
  const onAnalyzeEvidence = async () => {
    ...
    try {
      ...
      if (isMissingGeminiKeyError(err)) {
        ...
        setErrorMessage(`No se pudo ejecutar el análisis: ${err.message}`);
      } else {
        setErrorMessage('No se pudo ejecutar el análisis por un error inesperado.');
      }
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-bold">Módulo de Auditoría de Evidencias (PoC)</h1>
        <button className="px-4 py-2 bg-[#4F6EF7] text-white rounded-lg text-[13px] font-medium">
          Analizar Nueva Evidencia
        </button>
      </div>
        ...
        <Card title="📤 Subir Evidencia">
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              ...
              >
                {isLoading ? 'Analizando…' : '🔍 Analizar Evidencia'}
              </button>
              <button
                onClick={onClear}
        ...
        <Card title="📋 Resultado del Análisis">
          {errorMessage && (
            <div className="mb-4 p-4 bg-[#FEECEE] dark:bg-[#2A1214] border border-[#F4B4B8] dark:border-[#5A1E22] rounded-lg">
              <div className="text-sm font-semibold text-[#A1242A] dark:text-[#F06669]">Error</div>
          ...
          {auditResponse?.veredicto === 'NO CUMPLE' && (
            <div className="mb-4 p-4 bg-[#E5484D] rounded-lg flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-white font-bold text-sm">VEREDICTO: NO CUMPLE</div>
                  <div className="text-white/80 text-xs">Se requiere acción correctiva</div>
                </div>
              </div>
            </div>
          )}
          ...
          {auditResponse?.veredicto === 'CUMPLE' && (
            <div className="mb-4 p-4 bg-[#1DB954] rounded-lg flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-white font-bold text-sm">VEREDICTO: CUMPLE</div>
                  <div className="text-white/80 text-xs">No se requieren acciones</div>
                </div>
              </div>
            </div>
          )}
            ...
            <pre className="text-[#E4E7EE] text-xs font-mono leading-relaxed">
              {auditResponse
                ...
                : isLoading
                  ...
                  : '{\n  "status": "Aún no hay resultado. Ingresa evidencia y presiona Analizar."\n}'}
            </pre>
          </div>
        </Card>
      </div>
