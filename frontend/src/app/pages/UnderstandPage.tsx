                ...
                <div className="flex gap-2">
                  <ActionButton
                    ...
                    onClick={async () => {
                      try {
                        ...
                        if (!actions.can_create_document) {
                          Toast.error('No es posible crear documentación para esta fase')
                          return
                        }
                        if (onNavigate) onNavigate(`documents?create=${p.id}`)
                      } catch (err) {
                        Toast.error('Error validando la acción')
                      }
                    }}
                    variant="primary"
                  />
                  ...
                  <ActionButton
                    ...
                    onClick={async () => {
                      try {
                        const actions = await getGapActions(p.id)
                        if (!actions.can_modify_document) {
                          Toast.error('No es posible modificar documentación para esta fase')
                          return
                        }
                        if (onNavigate) onNavigate(`documents?edit=${p.id}`)
                      } catch (err) {
                        Toast.error('Error validando la acción')
                      }
                    }}
                  ...
                  <ActionButton
                    ...
                    onClick={async () => {
                      try {
                        const actions = await getGapActions(p.id)
                        if (!actions.can_audit) {
                          Toast.error('No es posible auditar este hallazgo')
                          return
                        }
                        if (onNavigate) onNavigate(`audit?phase=${p.id}`)
                      } catch (err) {
                        Toast.error('Error validando la acción')
                      }
                    }}
